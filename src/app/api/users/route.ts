...
export async function GET(request: NextRequest) {
    try {
      const adminToken = verifyToken(request);
        ...
        const usersByGovernorateResponse = usersByGovernorate.map(userByGovernorate => ({
      ...
      const countUsersByGovernorate =  users.length;
        ...
        if(countUsers < 1 || countUsersByGovernorate < 1){
          return NextResponse.json({message: "لا يوجد حسابات متاحة"}, {status: 404})
        }
    ...
    try {
        ...
        const newUser = await prisma.user.create({
          data: {
            ...
            avatar: body.avatar,
            isActive: body.role === Role.TECHNICAL ||body.role === Role.ADMIN ||body.role === Role.SUBADMIN ? false : true,
            role: (body.role as Role) || Role.USER,
            customer:  (body.role !== Role.SUBADMIN && body.role !== Role.ADMIN && body.role !== Role.TECHNICAL) ? { create: {} } : undefined,
            technician: body.role === Role.TECHNICAL  ? { create: {
              
              specialization: body.specialization,
              services: body.services,
              
            } } : undefined,
            subadmin: body.role === Role.SUBADMIN ? { create: {
